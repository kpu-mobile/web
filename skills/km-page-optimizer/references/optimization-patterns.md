...
**优化后**:
```vue
<KmButton @click="handleClick">提交</KmButton>

<KmButton :loading="loading">提交</KmButton>
```
...
**优化后**:
```vue
<KmInput v-model="username" placeholder="请输入用户名" />
```
...
<template>
  <KmButton @click="visible = true">打开对话框</KmButton>
  <KmModal v-model="visible" title="标题">
    内容
  </KmModal>
</template>
```
...
**优化后**:
```vue
<KmDrawer v-model="open" title="设置">
  内容
</KmDrawer>
```

> 注意：基础版只有 `KmDrawer`，不支持 `KmSpringDrawer`（专业版特性）。
...
```vue
<script setup>
import { useKmLoading } from '@/ui/components/KmLoading'

async function fetchData() {
  const { close } = useKmLoading({ text: '加载中...' })
  try {
    await api.getData()
  }
  finally {
...
**优化后**:
```vue
<KmCheckbox v-model="checked">同意协议</KmCheckbox>
```
...
**优化后**:
```vue
<KmSelect v-model="selected" :options="options" placeholder="请选择" />
```
...
**优化后**:
```vue
<KmAvatar :src="user.avatar" :alt="user.name" />
```

### 数字统计动画优化

> 注意：基础版不包含 `KmAnimatedCountTo` 等数字动画组件（专业版特性），请使用原生方式或第三方库实现。

## 常见场景映射

| 场景 | 原实现 | 框架组件 |
|------|--------|----------|
| 卡片容器 | 自定义 card div | `KmCard` |
| 按钮 | 原生 `<button>` | `KmButton` |
| 输入框 | 原生 `<input>` | `KmInput` |
| 下拉选择 | 原生 `<select>` | `KmSelect` |
| 开关 | 自定义 checkbox | `KmSwitch` |
| 弹窗 | 自定义遮罩弹层 | `KmModal` / `useKmModal()` |
| 抽屉 | 自定义侧边栏 | `KmDrawer` |
| 加载中 | 自定义 loading | `useKmLoading()` |
| 消息提示 | alert / 自定义 toast | `kmToast` |
| 标签页 | 手动切换 active | `KmTabs` |
| 头像 | img + fallback | `KmAvatar` |
