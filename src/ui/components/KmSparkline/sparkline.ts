...
function getY(max: number, height: number, diff: number, value: number) {
  return Number.parseFloat((height - (value * height / max) + diff).toFixed(2))
}
...
export function sparkline(svg: any, entries: any, options: any) {
  ...
  // rendering offset.
  const strokeWidth = Number.parseFloat(svg.attributes['stroke-width'].value)
  ...
  const width = Number.parseFloat(svg.attributes.width.value) - spotDiameter * 2

  // Get the SVG element's full height.
  // This is used
  const fullHeight = Number.parseFloat(svg.attributes.height.value)
