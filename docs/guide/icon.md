...
推荐去[阿里巴巴矢量图标库](https://www.iconfont.cn/)下载图标，将准备好的 SVG 图标文件放到 `apps/<app>/src/assets/icons/` 目录下，然后在页面中就可以通过 `KmIcon` 组件使用了，name 就是 svg 的文件名。
...
当然这么使用并没有很方便，依旧还是需要手动导入一个 Icon 组件。如果你也觉得麻烦的话，那么你可以使用 `KmIcon` 组件来展示，框架已经帮你做好的所有处理。
