...
- 根目录负责执行 `eslint` 和 `stylelint`
- 各应用目录只保留 `vue-tsc` 类型检查
- 执行 `pnpm lint` 时，会先在各应用中执行 `vue-tsc`，再在根目录完成 `eslint`、`stylelint`
...
此外，如果 git 仓库目录和框架目录并非同一个，则需要在 `package.json` 中修改 `postinstall` 脚本，切换到 git 所在目录。例如 git 目录是 `project/` ，而框架目录是 `project/kpu-mobile/` ，则在 `package.json` 里找到 `simple-git-hooks` 配置并修改：

```json {2}
"simple-git-hooks": {
  "pre-commit": "cd ./kpu-mobile/ && pnpm lint-staged",
  "preserveUnused": true
}
```
