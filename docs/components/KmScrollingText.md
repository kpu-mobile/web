...
<template>
  <KmScrollingText :items="announcements" />
</template>
```
...
```vue
<KmScrollingText :items="hotSearches">
  <template #item="{ item, index }">
    <div class="flex items-center gap-2">
      <span :class="getRankClass(index)">{{ index + 1 }}</span>
      <span>{{ item }}</span>
      <KmBadge v-if="index < 3" value="hot" />
    </div>
  </template>
</KmScrollingText>
```
...
<template>
  <KmScrollingText :items="links" @item-click="handleItemClick" />
</template>
```
...
```vue
<!-- 鼠标移入时自动暂停滚动，移出后继续 -->
<KmScrollingText :items="messages" />
```
