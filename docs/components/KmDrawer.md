...
<template>
  ...
  <KmDrawer v-model="open" title="抽屉标题">
    <div>抽屉内容</div>
  </KmDrawer>
</template>
```
...
<template>
  <KmButton @click="open = true">打开抽屉</KmButton>
  <KmDrawer v-model="open" title="抽屉标题" description="抽屉描述">
    <div>这是抽屉内容</div>
  </KmDrawer>
</template>
```
...
<template>
  <div class="flex gap-2">
    <KmButton @click="drawers.top = true">上方抽屉</KmButton>
    <KmButton @click="drawers.bottom = true">下方抽屉</KmButton>
    <KmButton @click="drawers.left = true">左侧抽屉</KmButton>
    <KmButton @click="drawers.right = true">右侧抽屉</KmButton>
  </div>

  <KmDrawer v-model="drawers.top" side="top" title="上方抽屉">内容</KmDrawer>
  <KmDrawer v-model="drawers.bottom" side="bottom" title="下方抽屉">内容</KmDrawer>
  <KmDrawer v-model="drawers.left" side="left" title="左侧抽屉">内容</KmDrawer>
  <KmDrawer v-model="drawers.right" side="right" title="右侧抽屉">内容</KmDrawer>
</template>
```
...
<template>
  <KmDrawer v-model="open" title="编辑" :before-close="handleBeforeClose">
    <form>
...
<template>
  <KmDrawer v-model="open" title="提交表单" :loading="loading" :confirm-button-loading="loading" @confirm="handleSubmit">
...
```vue
<template>
  <KmDrawer v-model="open" title="大抽屉" content-class="sm:max-w-2xl">
    <div>这是一个较宽的抽屉</div>
  </KmDrawer>
</template>
```
...
```vue
<template>
  <KmDrawer v-model="open" title="居中抽屉" centered show-cancel-button>
