...
```vue
<template>
  <KmScrollArea horizontal class="w-96">
...
```vue
<template>
  <KmScrollArea :scrollbar="false" class="h-64">
    <div>内容（无滚动条）</div>
  </KmScrollArea>
</template>
```
...
```vue
<template>
  <KmScrollArea :mask="true" class="h-64">
    <div class="space-y-2">
...
<template>
  <KmScrollArea class="h-64" @on-scroll="onScroll">
    <div>滚动内容...</div>
  </KmScrollArea>
</template>
```
...
```vue
<script setup lang="ts">
const scrollRef = ref<InstanceType<typeof KmScrollArea>>()
...
<template>
  <KmScrollArea ref="scrollRef" class="h-64">
    <div class="space-y-2">
      <p v-for="i in 50" :key="i">内容 {{ i }}</p>
    </div>
  </KmScrollArea>
  ...
  <div class="flex gap-2 mt-4">
    <KmButton @click="scrollToTop">回到顶部</KmButton>
    <KmButton @click="scrollToBottom">滚动到底部</KmButton>
  </div>
</template>
```
