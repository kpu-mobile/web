...
<template>
  ...
  <!-- 填充风格，带渐变背景和阴影 -->
  <KmTrend value="12.5%" variant="filled" />
  
  <!-- 柔和风格，带浅色背景 -->
  <KmTrend value="12.5%" variant="soft" />
  
  <!-- 边框风格 -->
  <KmTrend value="12.5%" variant="outline" />
</template>
```
...
```vue
<template>
  <KmTrend value="12.5%" size="small" />
  <KmTrend value="12.5%" size="medium" />
  <KmTrend value="12.5%" size="large" />
</template>
```
...
```vue
<template>
  <KmTrend value="12.5" type="up" suffix="%" />
  <KmTrend value="¥1,234" type="up" prefix="↑ " />
  <KmTrend value="$2,345.00" prefix="$" suffix=".00" variant="filled" />
</template>
```
...
<template>
  <!-- 错误率下降是好事，显示绿色 -->
  <KmTrend value="5.2%" type="down" reverse variant="filled" />
  
  <!-- 成本下降是好事，显示绿色 -->
  <KmTrend value="¥2,000" type="down" reverse prefix="节省 " variant="soft" />
</template>
```
  ...
  <table class="w-full">
    ...
    <tbody>
      <tr v-for="stock in stocks" :key="stock.name">
        ...
        <td>{{ stock.price }}</td>
        <td>
          <KmTrend :value="`${Math.abs(stock.change)}%`" :type="stock.type" />
        </td>
      </tr>
    </tbody>
  </table>
...
<template>
  <div>
    ...
    <!-- 成本下降是好事，使用 reverse -->
    <KmTrend value={`${costData.change}%`} type="down" reverse prefix="↓ 优化 " />
  </div>
</template>
```
