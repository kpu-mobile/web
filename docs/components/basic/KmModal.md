...
<template>
  <KmButton @click="open = true">打开对话框</KmButton>
  <KmModal v-model="open" title="对话框标题">
    <div>对话框内容</div>
  </KmModal>
</template>
```
...
<template>
  <KmButton @click="open = true">打开对话框</KmButton>
  <KmModal v-model="open" title="对话框标题" description="这里是一段描述">
    <div>这是对话框内容</div>
  </KmModal>
</template>
```
...
<template>
  <div class="flex gap-2">
    <KmButton @click="successOpen = true">成功提示</KmButton>
    <KmButton @click="errorOpen = true">错误提示</KmButton>
  </div>

  <KmModal v-model="successOpen" title="操作成功" icon="success" align-center border>
    <div>您的操作已成功完成</div>
  </KmModal>

  <KmModal v-model="errorOpen" title="操作失败" icon="error" align-center border>
    <div>操作失败，请稍后重试</div>
  </KmModal>
</template>
```
...
```vue
<template>
  <KmModal v-model="open" title="居中对话框" align-center>
...
```vue
<template>
  <KmModal v-model="open" title="内容居中" center>
...
```vue
<script setup lang="ts">
import { useKmModal } from '@kpu-mobile/components'
...
<template>
  <KmModal v-model="open" title="编辑" :before-close="handleBeforeClose">
    <form class="flex flex-col gap-4">
      <KmInput placeholder="请输入..." />
    </form>
  </KmModal>
</template>
```
...
<template>
  <KmModal v-model="open" title="提交" :loading="loading" :confirm-button-loading="loading" @confirm="handleSubmit">
    <form class="flex flex-col gap-4">
      <KmInput placeholder="输入内容" />
    </form>
  </KmModal>
</template>
```
...
```vue
<template>
  <KmModal v-model="open">
    <template #header>
      <div class="flex-between">
        <span class="font-bold">自定义标题</span>
        <KmIcon name="i-lucide:info" />
      </div>
    </template>
    <div>对话框内容</div>
    <template #footer>
      <div class="flex gap-2">
        <KmButton variant="outline">取消</KmButton>
        <KmButton>保存</KmButton>
      </div>
    </template>
  </KmModal>
</template>
...
```vue
<template>
  <KmModal v-model="open" title="大对话框" class="sm:max-w-2xl">
    <div>这是一个较宽的对话框</div>
  </KmModal>
</template>
```
...
<template>
  <div class="flex gap-2">
    <KmButton @click="showAlert">信息提示</KmButton>
    <KmButton @click="showSuccess">成功提示</KmButton>
    <KmButton @click="showConfirm">确认对话框</KmButton>
    <KmButton @click="showError">错误提示</KmButton>
    <KmButton @click="showWarning">警告提示</KmButton>
  </div>
</template>
```
...
6. **函数式调用**：`useKmModal()` 提供 `info`、`success`、`warning`、`error`、`confirm` 方法
