...
<template>
  <KmTimeline :data="data" class="h-400">
    <template #default="{ item }">
      <KmCard>{{ item.content }}</KmCard>
    </template>
  </KmTimeline>
...
<template>
  <KmTimeline :data="projectMilestones">
    <template #default="{ item }">
      <KmCard>
        <template #header>
...
<template>
  <div>
    ...
    <KmButton @click="addActivity('新活动', '当前用户')">添加活动</KmButton>
  </div>
</template>
```
