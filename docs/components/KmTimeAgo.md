...
```vue
<template>
  <KmTimeAgo :date="new Date()" :show-second="true" />
</template>
```
    ...
    <div v-for="comment in comments" :key="comment.id">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm text-muted-foreground">
          <KmTimeAgo :date="comment.createdAt" />
        </span>
      </div>
...
<template>
  <!-- 最大显示到"天" -->
  <KmTimeAgo :date="oldDate" max="day" />
...
<template>
  <div>
    当前时间：<KmTimeAgo :date="now" :update-interval="5000" />
  </div>
</template>
```
