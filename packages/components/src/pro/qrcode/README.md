...
<template>
  <KmQrcode :value="qrValue" :size="200" />
</template>
```
...
```vue
<KmQrcode :value="'https://example.com'" :size="200" />
```
...
<template>
  <!-- 本地图片 Logo -->
  <KmQrcode :value="url" :size="200" :logo="logoSvg" />
  
  <!-- 网络图片 Logo -->
  <KmQrcode :value="url" :size="200" logo="https://example.com/logo.png" />
</template>
```
...
```vue
<!-- 加载中状态 -->
<KmQrcode :value="url" status="loading" />

<!-- 已过期状态 -->
<KmQrcode :value="url" status="expired" />

<!-- 自定义状态渲染 -->
<KmQrcode :value="url" status="scanned">
...
```vue
<script setup lang="ts">
const qrRef = ref<InstanceType<typeof KmQrcode>>()
...
<template>
  <KmQrcode ref="qrRef" :value="url" />
  <KmButton @click="downloadQr">下载二维码</KmButton>
</template>
```
