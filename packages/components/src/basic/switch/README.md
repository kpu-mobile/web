...
```vue
<template>
  <KmSwitch v-model="enabled" disabled />
</template>
```
...
<template>
  <KmSwitch v-model="enabled" :before-change="beforeChange" />
</template>
```
...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      <span>邮件通知</span>
      <KmSwitch v-model="settings.emailNotification" />
    </div>
    <div class="flex items-center justify-between">
      <span>短信通知</span>
      <KmSwitch v-model="settings.smsNotification" />
    </div>
    <div class="flex items-center justify-between">
      <span>推送通知</span>
      <KmSwitch v-model="settings.pushNotification" />
    </div>
  </div>
</template>
```
