...
function resolveOptions(options: CopyrightOptions = {}) {
  return {
    edition: options.edition ?? '专业版',
    website: options.website ?? 'https://kpu-mobile.kpui.cn',
  }
}
...
function createTerminalInfoPlugin(options: CopyrightOptions = {}): PluginOption {
  ...
  return {
    ...
    async buildStart() {
      ...
      console.log(
        boxen(
          `${bold(green(`由 ${bgGreen('Kpu-mobile')} 驱动`))}\n\n${underline(website)}\n\n当前使用：${cyan(edition)}`,
          {
            padding: 1,
            margin: 1,
            borderStyle: 'double',
...
if ((navigator.language).toLowerCase() === 'zh-cn') {
  console.info('%c由%cKpu-mobile%c驱动%c👉%c${website}', ${JSON.stringify(subStyle)}, ${JSON.stringify(mainStyle)}, ${JSON.stringify(subStyle)}, ${JSON.stringify(iconStyle)}, ${JSON.stringify(linkStyle)})
}
else {
  console.info('%cPowered by %cKpu-mobile%c%c👉%c${website}', ${JSON.stringify(subStyle)}, ${JSON.stringify(mainStyle)}, ${JSON.stringify(subStyle)}, ${JSON.stringify(iconStyle)}, ${JSON.stringify(linkStyle)})
}
